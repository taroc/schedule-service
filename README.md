...
- `POST /api/matching/global` - グローバルマッチング実行（ダブルブッキング防止）
...
### 日程マッチングモード
- **連続日程 (consecutive)**: 連続した日程でのみマッチング（デフォルト）
- **柔軟日程 (flexible)**: 連続でなくても必要日数を満たせばマッチング
- **期間指定 (within_period)**: 指定期間内で必要日数を確保

### 優先度システム
- **高 (high)**: 最優先でマッチング処理
- **中 (medium)**: 標準優先度（デフォルト）
- **低 (low)**: 最低優先度

### ダブルブッキング防止
- **グローバルマッチング**: 全イベントを考慮した競合解決
- **優先度ベース**: 高優先度イベントを優先的に成立
- **先着順**: 同優先度では作成日時の早い順

### 実行タイミング
- ✅ **ユーザーがイベントに参加した時**
- ✅ **ユーザーがスケジュールを更新した時**
- ✅ **グローバルマッチング** (`POST /api/matching/global`)
- 🔧 **定期バッチ処理**（実装可能）
